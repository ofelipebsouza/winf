...
import { ArrowLeft, Briefcase, MessageCircle, LayoutDashboard, Layers, Gem } from 'lucide-react';
...
const features = [
  {
    ...
    title: "Plataforma Connect",
    desc: "Painel exclusivo para cotação em escala, aprovação de crédito corporativo B2B e emissão de ARTs instantâneas.",
  },
  {
    icon: Layers,
    title: "Hub de Engenharia",
    desc: "Acesso direto aos N1 da WINF para elaboração de especificações técnicas para licitações e construtoras.",
  },
  {
    icon: Gem,
    title: "Linha Architect Pro",
    desc: "Acesso restrito The Vault. Portfólio de películas arquitetônicas da linha Elite com bloqueio IR/UV superior.",
  },
];

const LandingPartners: React.FC<LandingPartnersProps> = ({ onBack }) => {
  useEffect(() => {
    window.scrollTo(0, 0);
  }, []);
  ...
  return (
    <div className="min-h-screen bg-[#000000] text-white selection:bg-white selection:text-black">
      ...
      <nav className="fixed top-0 w-full z-50 p-6 flex justify-between items-center bg-gradient-to-b from-black to-transparent pointer-events-none">
        <button
          onClick={onBack}
          className="pointer-events-auto flex items-center gap-2 text-xs md:text-xs font-bold uppercase tracking-widest text-white/50 hover:text-white transition-colors"
        >
          <ArrowLeft size={14} /> Retornar
        </button>
        <div className="font-medium tracking-widest text-sm text-white">
          WINF™ <span className="text-white/50">PARTNERS</span>
        </div>
      </nav>
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden pt-20 pb-20">
        ...
        <div className="container mx-auto px-6 relative z-10 flex flex-col items-center text-center">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 border border-white/20 bg-transparent text-xs md:text-xs font-bold uppercase tracking-[0.4em] text-white/70 mb-8">
          ...
          <div className="flex flex-col sm:flex-row gap-4 w-full max-w-xl mx-auto">
            <button
              ...
              className="flex-1 py-5 bg-white text-black shrink-0 text-xs md:text-xs font-bold uppercase tracking-[0.3em] hover:bg-zinc-200 transition-all flex items-center justify-center gap-3"
            >
              <MessageCircle size={16} /> Submeter Dossiê
            </button>
            <div className="flex-1 flex flex-col justify-center items-center px-6 py-5 border border-white/20 bg-transparent text-xs md:text-xs font-bold uppercase tracking-widest text-white backdrop-blur-md">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-1 px-1">
          {galleryImages.map((src, i) => (
            <div key={i} className="aspect-[3/4] relative overflow-hidden group border border-white/5">
              <div
                className="absolute inset-0 bg-cover bg-center transition-transform duration-[4s] group-hover:scale-105 grayscale group-hover:grayscale-0 opacity-80"
      ...
      <section className="py-32 border-b border-white/10 bg-black">
        <div className="container mx-auto px-6">
          <h2 className="text-3xl md:text-5xl font-black uppercase tracking-tighter mb-20 text-center text-white">
            Domínio B2B.
          </h2>
      ...
      <section className="py-40 bg-[#000000] relative overflow-hidden">
        <div
          className="absolute inset-0 z-0 bg-[url('https://images.unsplash.com/photo-1503376780353-7e6692767b70?auto=format&fit=crop&q=80')] bg-cover bg-center opacity-[0.03] grayscale"
        ></div>

        <div className="container mx-auto px-6 text-center relative z-10 max-w-3xl">
          <div className="text-xs md:text-xs font-bold uppercase tracking-[0.5em] text-white/40 mb-8">
          ...
          <button
            onClick={handleWhatsApp}
            className="mx-auto px-12 py-5 border border-white/30 text-white shrink-0 text-xs md:text-xs font-bold uppercase tracking-[0.3em] hover:bg-white hover:text-black transition-all flex items-center justify-center gap-3 backdrop-blur-md"
          >
